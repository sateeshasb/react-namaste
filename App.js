...
import './index.css'
  ...
  return (
        <div className='header'>
           <div className='logo-container'>
              <img className='logo' src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcThCagRLYkLeTBEd0qc-6FLb6lnyxJXaBw_Fw&s' alt='logo' />
           </div>
 ...
 const RestaurantCard = (props) => {
  ...
  const{cloudinaryImageId, 
    ...
    avgRating,
    costForTwo,
   deliveryTime} = reData.info;
  return(
    <div className='res-card'>
      <img className='res-logo' src={`https://dineout-media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto/${cloudinaryImageId}`} alt='rest-logo' />
      <h3>{ name}</h3>
      <h4> { cuisines.join(", ")}</h4>
      <h4>{avgRating}</h4>
      <h4>{ costForTwo}</h4>
